...
class _CreativeScreenState extends State<CreativeScreen> {
  ...
  final List<Map<String, dynamic>> _creativeTypes = [
    {'label': 'Painting', 'icon': Icons.palette_outlined, 'color': Color(0xFFFF6767)},
    {'label': 'Writing', 'icon': Icons.edit_note_outlined, 'color': Color(0xFF67B2FF)},
    {'label': 'Music', 'icon': Icons.music_note_outlined, 'color': Color(0xFFB267FF)},
    {'label': 'Design', 'icon': Icons.design_services_outlined, 'color': Color(0xFF67FFB2)},
    {'label': 'Coding', 'icon': Icons.code_rounded, 'color': Color(0xFFFFD167)},
    {'label': 'Crafting', 'icon': Icons.brush_outlined, 'color': Color(0xFF67B2FF)},
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return QuickLogScaffold(
      title: 'Creative Work',
      subtitle: 'Log your creative progress',
      topImage: Image.asset(
        'Assets/onboarding_image_2.png',
        fit: BoxFit.cover,
        errorBuilder: (context, error, stackTrace) => Container(
          color: isDark ? const Color(0xFF1A1A2E) : const Color(0xFFE5E7EB),
          child: Icon(Icons.palette_outlined, color: isDark ? Colors.white24 : Colors.black26, size: 48),
        ),
      ),
      saveButton: QuickLogSaveButton(
        label: 'Save Creative Log',
        isReady: _selectedType != null || _activityController.text.isNotEmpty,
        isSaving: _isSaving,
      ...
      body: Column(
        ...
        children: [
          SectionHeader(title: 'What are you working on?', isDark: isDark),
          const SizedBox(height: 16),
          _buildTypeGrid(isDark, textColor),
          const SizedBox(height: 32),
          SectionHeader(title: 'Custom Activity Name', subtitle: 'e.g., Oil Painting, Blog Post', isDark: isDark),
          const SizedBox(height: 16),
          _buildActivityField(isDark, textColor, subTextColor),
          const SizedBox(height: 32),
          SectionHeader(title: 'Duration', subtitle: 'How long did you create?', isDark: isDark),
          const SizedBox(height: 16),
    ...
    return GlassCard(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          final type = _creativeTypes[index];
  ...
  Widget _buildActivityField(bool isDark, Color textColor, Color subTextColor) {
    return GlassCard(
  ...
  Future<void> _handleSave() async {
    ...
    try {
      final log = ActivityLog(
        ...
        userId: user.uid,
        type: 'creative',
        value: _selectedType ?? _activityController.text,
        duration: _duration,
        notes: _selectedType != null ? _activityController.text : '',
      ...
      if (mounted) {
        CustomPopup.show(
          context: context,
          title: 'Error',
          message: 'Failed to save creative log: $e',
          primaryColor: Colors.redAccent,
        );
      }
    } finally {
